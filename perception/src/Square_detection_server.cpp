...
float detect_box_height(const Cloud& cloud, float x, float y)
{
  ...
  float min = x - 0.01;
  float max = x + 0.01;
  cout<<"range for x is: "<<min<<" to "<<max<<endl;
             ...
             perception_msgs::SquareDetection::Response &res)
{
  ...
  { 
    ...
    if(squares.size() != 0){
      ...
      res.Square_pose.pose.position.x = sensor_cloud_ptr->at(pixel_x, pixel_y).x;
      res.Square_pose.pose.position.y = sensor_cloud_ptr->at(pixel_x, pixel_y).y;
      res.Square_pose.pose.position.z = detect_box_height(*sensor_cloud_ptr, res.Square_pose.pose.position.x, res.Square_pose.pose.position.y);
      //res.Square_pose.pose.position.z = sensor_cloud_ptr->at(pixel_x, pixel_y).z;
